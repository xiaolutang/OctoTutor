...
章鱼哥不是一个什么都答的通用聊天机器人，它先只服务高中数学，而且知识来源限定在固定教材里。这样做不是因为通用问答不重要，而是因为数学解题最怕“看起来讲得很顺，但依据不清楚”。先把知识范围收窄，后面才有机会把回答质量做扎实。

#### 启发式问答

我不希望它变成一个直接吐答案的工具。学生输入题目后，它应该更像一个助教：先判断题目考什么知识点，再拆出关键步骤，必要时反问一句，提醒容易错的地方，最后让学生知道这道题为什么这样做。
