...
这不是在增强模型能力，而是在补产品能力。RAG、智能体和流式输出解决的是“能不能回答”；多对话管理解决的是“用户能不能长期使用”。
...
游标分页不是为了显得复杂，而是为了避免页码分页在数据变化时出现重复或漏项。对话列表按时间倒序排列，如果用户一边翻页一边产生新对话，传统 page/offset 很容易不稳定。用 `updated_at + id` 做游标，排序会更可控。
...
也就是说，前端不能把 `done` 理解成“这条 SSE 流的最后一帧”，只能理解成“回答内容已经生成结束”。标题更新是回答结束后的附加事件。
...
这里原本希望 `updated_at` 表示最近活跃时间，也就是只在有新消息产生时更新。但当前实现里，`Conversation` ORM model 给 `updated_at` 配了 `onupdate`，所以重命名、置顶、取消置顶以及标题生成这类更新操作也会刷新 `updated_at`。

这不影响这一期主链路跑通，但它会影响“最近活跃”的严格语义。后面如果要让排序完全只反映消息活跃时间，可以把“最近消息时间”和“记录更新时间”拆成两个字段。
...
从当时的验收记录看，这一期后端和前端都做了比较完整的测试：后端任务、前端任务和最终批量验收都通过了；其中后端 105 条、前端 241 条，合计 346 条测试没有新增失败。`npm run build` 当时受本地 auth SDK 符号链接影响失败，被标记为环境问题，不是这次对话管理逻辑本身的问题。

---

## 八、这一期真正补上的是什么

这一期看起来是在做“对话列表”，但它补上的其实是一个 AI 产品长期使用的基本骨架。

单次问答能跑，只能说明能力链路成立；多对话能管理，才开始接近一个真实产品。

到这里，章鱼哥解题已经不只是能回答、能流式显示、能恢复当前对话，而是开始具备“学习空间”的雏形：用户可以围绕不同题目和知识点建立多条对话，之后再回来继续查看和整理。

下一步再继续做多轮追问、上下文压缩、问题改写和更细的评估时，这些能力就不再只是散落的接口，而是能落到具体的用户对话里。
